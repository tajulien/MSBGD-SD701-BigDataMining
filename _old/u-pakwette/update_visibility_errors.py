...
def envoi_sql(record_to_insert):
    ...
    try:
        ...
        %s)  
        ON DUPLICATE KEY UPDATE visibilite=%s"""
        cursor = connection.cursor(prepared=True)
        result = cursor.executemany(sql_insert_query, record_to_insert)
        with open('whatididtonight.txt', 'a+') as fd:
            fd.write(sql_insert_query)
            fd.write('\n')
            fd.write(str(record_to_insert))
            fd.write('\n')
        connection.commit()
    ...
    for i in range(0, len(one_test[0])):
        ...
        if f'{i:02d}h' in one_test[0].keys():
            touple = (one_test[1] + f'-{i:02d}', date, f'{i:02d}', one_test[0][f'{i:02d}h']['temp'],
                      ...
                      one_test[0][f'{i:02d}h']['visi'], one_test[0][f'{i:02d}h']['visi'])
...
with open('visibility_errors.txt', 'r') as fd:
    lines = fd.readlines()
    lines = [line.rstrip()[:-3] for line in lines]
    lines = list(set(lines))
    # print(lines[0])
    # print(lines[0][:4])
    # print(re.sub("\d+", "", lines[0]))
    # print(lines[0][-2:])
    for elt in lines:
        parse_master(elt[:4], re.sub("\d+", "", elt), elt[-2:])
